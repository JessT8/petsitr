  ...
  before_action :authenticate_user!
    ...
    def update
        ...
        if @book.status == true
            ...
            if @booking.start_date == @timeslot.available_start_date && @booking.end_date < @timeslot.available_end_date
                @bookings = Booking.where(sitter: current_user.sitter).where('start_date OR end_date BETWEEN ? and ?', @booking.start_date, @booking.end_date)
                ...
                @bookings = Booking.where(sitter: current_user.sitter).where("start_date <= ? or end_date >= ?", @booking.start_date, @booking.end_date)
                @bookings.each do |booking|
                    booking.update(status: false)
